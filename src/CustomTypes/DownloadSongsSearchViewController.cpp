                ...
                if (pagelist.has_value() && !pagelist.value().GetDocs().empty()) {
                    ...
                    if(docs.size() == 1)
                    {
                        ...
                        [this, currentSearchIndex, playlistItem] {
                                BeatSaver::API::SearchPlaylistAsync(std::to_string(playlistItem.GetPlaylistId()), DownloadSongsSearchViewController::searchPage / ( 100 / ENTRIES_PER_PAGE),
                                                ...
                                                [this, currentSearchIndex, plist] {
                                                    if (currentSearchIndex == DownloadSongsSearchViewController::searchIndex) {
                                                        if (plist.has_value() && !plist.value().GetMaps().empty()) {
                                                            ...
                                                            int mapIndex = ( DownloadSongsSearchViewController::searchPage % ( 100 / ENTRIES_PER_PAGE) ) * ENTRIES_PER_PAGE;
                                                            for (int i = 0; i < ENTRIES_PER_PAGE; i++) {
